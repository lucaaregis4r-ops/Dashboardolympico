...
function onOpen() {
  SpreadsheetApp.getUi()
    .createMenu("Fisioterapia")
    .addItem("Criar/ajustar estrutura", "setupPhysioWorkbook")
...
function applyValidations_() {
  ...
  const injuries = listValues.filter((row) => row[0] === "lesao").map((row) => row[1]);
  ...
  treatment
    .getRange("I2:I")
    .setDataValidation(SpreadsheetApp.newDataValidation().requireValueInList(injuries, true).build());
  treatment
    .getRange("K2:K")
    .setDataValidation(SpreadsheetApp.newDataValidation().requireNumberBetween(0, 10).build());
}
...
function appendHistory_(movementType, treatmentRow) {
  ...
  historySheet.appendRow([
    ...
    treatmentRow[5],
    treatmentRow[6],
    movementType === "alta" ? "alta" : "em_tratamento",
    treatmentRow[7],
    treatmentRow[8],
    treatmentRow[9],
    treatmentRow[10],
