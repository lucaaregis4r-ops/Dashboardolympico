...
- Fonte de verdade: `src/client/` e `src/server/index.js`; os caminhos antigos citados no historico abaixo foram substituidos.
- Linux: `bash ABRIR-DASHBOARD.sh` inicia o servidor e abre o navegador; `--install-shortcut` instala uma entrada no menu de aplicativos, inclusive com Node instalado pelo nvm.
- A abertura do navegador acontece uma unica vez, na porta efetivamente disponivel. Falhas do `xdg-open` mostram o endereco para abertura manual.
- Conexoes: base principal de check-ins, fisioterapia por modalidade, psicologia nos relatorios e cinco planilhas suplementares de presenca (16 categorias).
- Atividade individual usa janela combinada de 20 dias, com conciliacao de nomes aprovada; atletas inativos permanecem identificados na interface.
- GitHub Pages inclui `data/attendance.json`; a verificacao exige as 16 categorias carregadas para evitar publicar uma sincronizacao incompleta.
- Publicacao no repositorio `lucaaregis4r-ops/Dashboardolympico`, com codigo-fonte e sincronizacao horaria via GitHub Actions.
- Versao 2.1.3 validada no Linux: 52 testes aprovados, 16 abas acessiveis, build com 281 atletas e 30 registros de fisioterapia. Teste de abertura confirmou caminhos com espacos/acentos, execucao fora da pasta do projeto e porta ocupada sem duplicar a abertura do navegador.
...
4. O CSV bruto e transformado em uma lista consolidada de atletas com:Ve check-ins
   - status atual
